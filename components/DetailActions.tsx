...
export function DetailActions({ exhibition, canUpdate, canDelete }: { exhibition: Exhibition; canUpdate: boolean; canDelete: boolean }) {
  ...
  return <div className="detail-actions"><button className="button detail-save" onClick={save}>{saved ? <Bookmark /> : <Heart />}{saved ? "Saved" : "Save"}</button>{canUpdate && <button className="button detail-edit" onClick={() => setEditing(true)}><Pencil /> Edit</button>}{canDelete && <div className="row-menu-wrap"><button className="icon-button" onClick={() => setMenu((v) => !v)} aria-label="More actions"><EllipsisVertical /></button>{menu && <div className="row-menu"><button className="danger" onClick={() => { setDeleting(true); setMenu(false); }}>Delete Exhibition</button></div>}</div>}{editing && canUpdate && <ReviewModal initial={exhibition} mode="edit" onClose={() => setEditing(false)} onSaved={() => router.refresh()} />}{deleting && canDelete && <Modal onClose={() => setDeleting(false)} label="Delete exhibition"><div className="delete-dialog"><div className="delete-icon"><Trash2 /></div><h2>Delete Exhibition?</h2><p>Are you sure you want to delete <strong>“{exhibition.name}”</strong>?<br />This action cannot be undone.</p><div><button className="button outline" onClick={() => setDeleting(false)}>Cancel</button><button className="button destructive" onClick={remove}>Delete Exhibition</button></div></div></Modal>}</div>;
}
