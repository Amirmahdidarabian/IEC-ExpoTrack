...
export default async function ExhibitionsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="app-page"><Header /><section className="database-hero"><div><p className="eyebrow">Global exhibition intelligence</p><h1>All Exhibitions</h1><p>Explore the world’s leading energy exhibitions, trade shows and conferences.</p></div><div className="stat-cards"><div><CalendarDays /><span><b>{result.stats.total}</b><small>Total Exhibitions</small></span></div><div><Globe2 /><span><b>{result.stats.countries}</b><small>Countries</small></span></div><div><Building2 /><span><b>{result.stats.industries}</b><small>Industries</small></span></div></div></section><div className="page-container"><ExhibitionExplorer result={result} /></div><Footer /></main>;
}
